...
try:
    ...
    cur.execute('''select hosts.host, hosts.name
                   from hosts_groups
                   inner join hosts
                   on hosts_groups.groupid=5 and hosts_groups.hostid=hosts.hostid;''')
    rows = cur.fetchall()
    if (len(rows) == 0): exit()
    for row in rows:
        ip = row[0]
        ...
        #update database
        update_sql=str("update hosts set name='" + dhname + "' where host='" + ip + "'")
